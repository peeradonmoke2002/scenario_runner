...
class SignalizedJunctionLeftTurn(JunctionLeftTurn):
    ...
    def _create_behavior(self):
        ...
        """
        sequence = py_trees.composites.Sequence("SignalizedJunctionLeftTurn")
        if self.route_mode:
        ...
        root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="SignalizedJunctionLeftTurn_Root")
        end_condition = py_trees.composites.Sequence("end_condition")
...
class NonSignalizedJunctionLeftTurn(JunctionLeftTurn):
    ...
    def _create_behavior(self):
        ...
        where, a flow of actors coming straight is present.
        """
        sequence = py_trees.composites.Sequence("NonSignalizedJunctionLeftTurn")
        if self.route_mode:
            sequence.add_child(HandleJunctionScenario(
                clear_junction=True,
                clear_ego_entry=True,
        ...
        root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="NonSignalizedJunctionLeftTurnRoot")
        end_condition = py_trees.composites.Sequence("end_condition")
        end_condition.add_child(WaitEndIntersection(self.ego_vehicles[0]))
        end_condition.add_child(DriveDistance(self.ego_vehicles[0], self._end_distance))
        root.add_child(end_condition)
