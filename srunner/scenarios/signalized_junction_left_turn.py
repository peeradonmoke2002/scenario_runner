...
class SignalizedJunctionLeftTurn(JunctionLeftTurn):
    ...
    def _create_behavior(self):
        ...
        """
        sequence = py_trees.composites.Sequence("SignalizedJunctionLeftTurn", memory=True)
        if self.route_mode:
        ...
        end_condition = py_trees.composites.Sequence("end_condition", memory=True)
        ...
        tl_freezer_sequence = py_trees.composites.Sequence("Traffic Light Behavior", memory=True)
...
class NonSignalizedJunctionLeftTurn(JunctionLeftTurn):
    ...
    def _create_behavior(self):
        ...
        where, a flow of actors coming straight is present.
        """
        sequence = py_trees.composites.Sequence("NonSignalizedJunctionLeftTurn", memory=True)
        if self.route_mode:
            sequence.add_child(HandleJunctionScenario(
                clear_junction=True,
                clear_ego_entry=True,
