...
class Accident(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        """
        root = py_trees.composites.Sequence("Accident", memory=True)
        if self.route_mode:
        ...
        behavior = py_trees.composites.Sequence("behavior", memory=True)
...
class AccidentTwoWays(Accident):
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Sequence("AccidentTwoWays", memory=True)
        if self.route_mode:
            total_dist = self._distance + self._first_distance + self._second_distance + 20
            root.add_child(LeaveSpaceInFront(total_dist))
...
class ParkedObstacle(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        """
        root = py_trees.composites.Sequence("ParkedObstacle", memory=True)
        if self.route_mode:
...
class ParkedObstacleTwoWays(ParkedObstacle):
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Sequence("ParkedObstacleTwoWays", memory=True)
        if self.route_mode:
            total_dist = self._distance + 20
            root.add_child(LeaveSpaceInFront(total_dist))
...
class HazardAtSideLane(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        """
        root = py_trees.composites.Sequence("HazardAtSideLane", memory=True)
        if self.route_mode:
        ...
        # End condition
        end_condition = py_trees.composites.Sequence("End Condition", memory=True)
        ...
        for actor, target_loc in zip(self.other_actors, self._target_locs):
            bicycle = py_trees.composites.Sequence("Bicycle behavior", memory=True)
        ...
        behavior = py_trees.composites.Sequence("Side lane behavior", memory=True)
...
class HazardAtSideLaneTwoWays(HazardAtSideLane):
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Sequence("HazardAtSideLaneTwoWays", memory=True)
        if self.route_mode:
            total_dist = self._distance + self._obstacle_distance + 20
            root.add_child(LeaveSpaceInFront(total_dist))
            root.add_child(ChangeRoadBehavior(extra_space=self._extra_space))
